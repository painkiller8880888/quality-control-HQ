...
- secret-scan-history: `workflow_dispatch`でGitleaks CLIによるrepository全履歴検査を別jobとして実行します。これはPR差分のrequired checkとは分離した手動検査です。
...
依存範囲を変更した場合は、リポジトリルートで次を実行して`requirements.lock`を更新し、lockも同じPRへ含めます。CIのbackend installとdependency-auditはこのlockだけを使用します。
