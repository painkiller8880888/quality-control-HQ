...
| S2-SH-06 | ADパスワード変更・期限切れ時のサービス資格情報更新運用 | 未実施 | — | 疑似本番Windows / AD | — | — | 個人アカウント利用の可否を判断し、変更手順、期限切れ検知、復旧時間を確認する |
| S2-HTTP-01 | 画面/APIからマスター更新を要求し、実HTTPで短時間に202を返す | 部分実施 | 2026-07-21 | 疑似本番HTTP / Windowsサービス / PostgreSQL | 既存の永続証跡付きA→B→A再試験3要求（2.109秒・2.076秒・2.078秒）に加え、同一canonical CSV・UNC 7 root・業務baseline条件で正式HTTPを再計測し、0.063秒で202を返した。Jobは`queued -> running -> succeeded`、試行1、621.408秒、canonical 10,807/1,001/113,876/3,304件、folder warning 0、更新後stable hash一致だった。1800秒timeoutの余裕は1178.592秒で、完了後もworker Running、active Job 0を確認した | 新Job `job_20260721131121_e8edaf53`、`runtime/pseudoprod/evidence/s2-http-fifo-dependency-20260721-130252/s2-http-01.json`、full backup `quality_prodlike_pre_http_fifo_dependency_20260721-130252.dump`（SHA-256 `13dda43e3c9f14f41da0ba5676fcf8b0f4b3b33c7c1c1aa7c1ba93440bd6969a`）。既存Job `job_20260717145144_6e6a67e8`、`runtime/pseudoprod/evidence/http-dedupe-20260717-145028` | 5秒未満は実測基準に留まり、正式な応答時間閾値・許容変動・timeout余裕の責任者承認がないため部分実施を維持する。新計測では前回1217.740秒から621.408秒へ短縮したが、UNC/I/O、同時負荷、キャッシュ、DB条件を含む性能基準の承認が必要 |
...
| S2-PAR-01 | 独立Job用の並列workerを第2段階で導入するか後段へ延期するか判断する | 未実施 | — | 設計・運用判断 | — | — | 安全性試験結果と待ち時間を基に、責任者、判断日、理由を記録する |
