...
- `3`: セッター。DBの`machine_assignment`テーブルにおいて、`machine_class`が`3`の機械に割り当てられたコード。`セッターフォルダ`\\192.168.1.210\@isk\★部門\④製造管理部\★品質保証\改正検査書\改正検査書\工程内検査\巡回検査\セッター\にファイルが存在するコード。
